...
export default function Metronome() {
  ...
  }, [getOrCreateAudioCtx, analyseMic]);
  ...
  useEffect(() => () => { stopMic(); stopScheduler(); }, []); // eslint-disable-line
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#F0EFE8",
      ...
      <div style={{ position: "relative", width: 320, height: 320 }}>
        <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%", overflow: "visible" }}>
          {Array.from({ length: NUM_STARS }).map((_, i) => {
            ...
            const isAccent = i === 0;
            let fill = "none", stroke = "#4ECDC4", glow = null;
            if (isBeat) {
            ...
            return (
              <g key={i} onClick={() => toggleStar(i)} style={{ cursor: i === 0 ? "default" : "pointer" }}>
                {isBeat && <path d={starPath(cx, cy, 9, 4)} fill={glow} opacity={0.2} style={{ filter: "blur(5px)" }} />}
                <path d={starPath(cx, cy)} fill={fill} stroke={stroke} strokeWidth={0.75}
                  style={{
        ...
        <button onClick={() => setIsPlaying(p => !p)}
          style={{
            ...
            background: isPlaying ? "linear-gradient(135deg, #FF6B35, #E8441A)" : "linear-gradient(135deg, #4ECDC4, #3DBDB5)",
            border: "none", cursor: "pointer",
            display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: isPlaying ? "0 0 0 6px rgba(255,107,53,0.2), 0 8px 32px rgba(255,107,53,0.45)" : "0 4px 20px rgba(78,205,196,0.35)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 28, marginTop: 24 }}>
        <button
          onMouseDown={e => { e.currentTarget.style.transform = "scale(0.92)"; startLongPress(-1); }}
          onMouseUp={e => { e.currentTarget.style.transform = "scale(1)"; stopLongPress(); }}
          onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; stopLongPress(); }}
          onTouchStart={e => { e.currentTarget.style.transform = "scale(0.92)"; startLongPress(-1); }}
          onTouchEnd={e => { e.currentTarget.style.transform = "scale(1)"; stopLongPress(); }}
          style={{ width: 62, height: 62, borderRadius: "50%", background: "linear-gradient(135deg, #4ECDC4, #3DBDB5)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(78,205,196,0.4)", outline: "none", transition: "transform 0.1s", WebkitTapHighlightColor: "transparent" }}
        >
          <svg width="22" height="4" viewBox="0 0 22 4"><rect x="0" y="0" width="22" height="4" rx="2" fill="white" /></svg>
        </button>

        <div onClick={handleTap} title="タップでテンポ設定"
          style={{ width: 110, textAlign: "center", fontSize: 64, fontWeight: 800, color: micActive ? "#2ECC71" : "#3DBDB5", cursor: "pointer", letterSpacing: -2, lineHeight: 1, fontFamily: "'Georgia', serif", textShadow: micActive ? "0 2px 12px rgba(46,204,113,0.35)" : "0 2px 8px rgba(61,189,181,0.2)", transition: "color 0.3s", WebkitTapHighlightColor: "transparent" }}
        >{bpm}</div>
        ...
        <button
          ...
          style={{ width: 62, height: 62, borderRadius: "50%", background: "linear-gradient(135deg, #4ECDC4, #3DBDB5)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(78,205,196,0.4)", outline: "none", transition: "transform 0.1s", WebkitTapHighlightColor: "transparent" }}
        >
          <svg width="22" height="22" viewBox="0 0 22 22"><rect x="0" y="9" width="22" height="4" rx="2" fill="white" /><rect x="9" y="0" width="4" height="22" rx="2" fill="white" /></svg>
        </button>
      </div>

      <div style={{ marginTop: 28, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        <button onClick={() => micActive ? stopMic() : startMic()}
          style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 24px", borderRadius: 32, background: micActive ? "linear-gradient(135deg, #1aad6c, #128a55)" : "linear-gradient(135deg, #4ECDC4, #3DBDB5)", border: "none", cursor: "pointer", color: "white", fontSize: 13, fontWeight: 700, letterSpacing: 1, boxShadow: micActive ? "0 4px 18px rgba(26,173,108,0.45)" : "0 4px 16px rgba(78,205,196,0.4)", transition: "all 0.25s ease", outline: "none", WebkitTapHighlightColor: "transparent" }}
        >
        ...
        {micActive && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            <div style={{ width: 180, height: 6, borderRadius: 3, background: "rgba(78,205,196,0.2)", overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: 3, width: `${micLevel * 100}%`, background: micLevel > 0.75 ? "linear-gradient(90deg, #2ECC71, #E74C3C)" : "linear-gradient(90deg, #2ECC71, #4ECDC4)", transition: "width 0.05s linear" }} />
            </div>
            {detectedBpm
              ? <span style={{ fontSize: 11, color: "#2ECC71", fontFamily: "monospace", fontWeight: 700, letterSpacing: 1 }}>検出: {detectedBpm} BPM</span>
              : <span style={{ fontSize: 11, color: "#9BB0AE", fontFamily: "monospace", letterSpacing: 1 }}>音楽を流してください…</span>
            }
          </div>
        )}
        ...
        {micStatus === "error" && (
          <div style={{ textAlign: "center", maxWidth: 260 }}>
            <span style={{ fontSize: 12, color: "#E74C3C", fontFamily: "monospace", fontWeight: 700 }}>マイクへのアクセスが拒否されています</span>
            <br />
            <span style={{ fontSize: 11, color: "#9BB0AE", lineHeight: 1.7 }}>ブラウザの設定からマイクの許可を変更してください。</span>
          </div>
        )}
      </div>
